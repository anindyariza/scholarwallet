...
app.post("/api/chat/auto-reply", async (req, res) => {
    try {
      ...
      if (!apiKey) {
        console.warn("No GEMINI_API_KEY found, falling back to local AI.");
        return res.json({ reply: generateLocalAiReply(message) });
      }

      // Construct history context if present
      let context = "";
      if (Array.isArray(history) && history.length > 0) {
        context = "Berikut riwayat percakapan sebelumnya:\n";
        history.slice(-10).forEach((h: any) => {
          const role = h.senderId === "admin" ? "Admin (Anda)" : "Mahasiswa";
          context += `- ${role}: ${h.text}\n`;
        });
        context += "\n";
      }

      const prompt = `${context}Mahasiswa bertanya: "${message}"\n\nBerikan jawaban bantuan finansial atau bantuan teknis aplikasi yang ramah, sopan, ringkas, dan jelas dalam Bahasa Indonesia.`;

      const systemInstruction = `Anda adalah "ScholarWallet AI Assistant", asisten keuangan pintar dan ramah.
Tugas Anda adalah:
1. Membantu mengelola anggaran, memberikan tips hemat, dan menjelaskan fitur aplikasi.
2. TERBUKA UNTUK MENJAWAB SEMUA PERTANYAAN mengenai keuangan sehari-hari, perencanaan keuangan, investasi, menabung, dan topik finansial lainnya.
3. Menangani kendala teknis atau laporan bug aplikasi secara sopan dan solutif.

Gunakan Bahasa Indonesia yang santun, bersahabat, terstruktur dengan baik (gunakan bullet points jika perlu). Jawablah secara langsung dan berikan motivasi keuangan yang baik.`;
      ...
      try {
        ...
        // AI request unsuccessful, triggering fallback mechanism
        
        // Fallback gracefully but inform the user if it's a quota issue
        if (geminiError.message && (geminiError.message.includes("429") || geminiError.message.toLowerCase().includes("quota"))) {
           ...
           // Fallback to local AI logic for basic queries if AI is completely down
      ...
      res.status(500).json({ error: error.message || "Gagal menghasilkan balasan AI" });
    }
  });
